...
// Net U2Netp::LoadModelByONNX(string onnx_path){
...
Net U2Netp::LoadModel(string modelFilePath, string modelType){
    try
    {
        ...
        start = clock();
        if(modelType == "onnx"){
            Net net = readNetFromONNX(modelFilePath);
        }
        else{
            Net net = readNet(modelFilePath);
        }
        end = clock();
        cout << "model loaded. spend " << end - start << "ms" << endl;
        return net;
...
Mat U2Netp::PredictByMat(Mat image){
    ...
    if(this->useGPU){
        ...
        Mat result_gpu = this->net.forward();
        // image_gpu.download(result);
    }
    else{
        this->net.setInput(image);
...
U2Netp::U2Netp(string modelPath, Size inputSize, bool useGPU, string modelFileType)
{
    ...
    if(getCudaEnabledDeviceCount() > 1 && useGPU){
